...
import main.java.nlp.similarity.BaseSimilarityStrategy;

public class ExtendedJaccardSimilarity extends BaseSimilarityStrategy {
	...
	protected void execute(Document doc) {
        ...
        for (int i=0;i<words.size();i++) {
        	...
        	for(int j=i+1; j<words.size();j++) {
        		...
        		double score=commonSentences.size()/((double)w1Sentences.size() + w2Sentences.size() - commonSentences.size());
