...
Una volta ottenuto il layout, si prosegue con l'applicazione di un algoritmo di \textit{clustering}, il quale suddivide le parole in più gruppi (\textit{cluster}). Ad ogni cluster viene assegnato un colore diverso e ogni colore corrisponde ad un diverso raggruppamento semantico. In generale, l'obiettivo del clustering è quello di ottenere che gli oggetti di ogni cluster siano il più possibile simili tra loro, o comunque che la loro correlazione sia più alta rispetto a quella con oggetti di cluster diversi.
\\ \\
L'algoritmo di clustering utilizzato in questa tesi e che verrà ora descritto, è una versione migliorata di uno dei più noti algoritmi in letteratura, cioè K-means++, proposto da Arthur et. al in\cite{arthur}.

\subsubsection{K-means}
K-means è un tipo di algoritmo \textit{prototype-based}, cioè basato sul concetto di prototipo, che è un elemento che rappresenta un cluster. Infatti, un cluster è una collezione di oggetti in cui ogni oggetto è più simile al prototipo di quel cluster rispetto ai prototipi di altri cluster. K-means definisce il prototipo in termini di centroide, spesso inteso come elemento medio di un gruppo di punti

La tecnica di clustering definita dal K-means è piuttosto semplice e intuitiva. In generale, dato un intero $K>0$ e un insieme di $n$ oggetti in un dataset $\chi$, si vogliono scegliere $K$ centroidi, tali da minimizzare la funzione obiettivo $\phi = \sum\limits_{i=1}^K \sum\nolimits_{x \in C_{i}} dist(c_{i},x) $, dove $C_{i}$ è l'i-esimo cluster e $dist(c_{i},x)$ è una qualche misura di prossimità tra il generico elemento $x \in C_{i} $ e il centroide dell'i-esimo cluster $C_{i}$. In pratica, ciò che si vuole ottenere, sono $k$ cluster in cui gli oggetti appartenenti ad ogni cluster siano il più possibile simili tra loro.
\\ \\
...
\item I passi 2 e 3 vengono eseguiti iterativamente finchè non ci sono ulteriori variazioni dei centroidi.
\end{enumerate}

Spesso i centroidi iniziali vengono scelti in modo randomico, ma sono possibili diverse scelte. In ogni caso, l'idea dell'algoritmo è quella di minimizzare, ad ogni esecuzione, la funzione $\phi$, tramite i passi 2 e 3 (i quali garantiscono il decremento di $\phi$), finchè non vengono osservate variazioni nei centroidi. Quando l'algoritmo termina, i centroidi dovrebbero aver identificato i raggruppamenti naturali degli elementi del dataset.

L'obiettivo dell'algoritmo, come detto, è quello di minimizzare la funzione obiettivo $\phi$. Tale funzione, misura la qualità del clustering tramite il calcolo della prossimità (o similarità) tra il centroide $c_{i}$ e gli elementi del cluster $C_{i}$. Solitamente, i punti fanno parte di uno spazio $n$-dimensionale e come misura di prossimità si utilizza la distanza euclidea. Ad ogni modo, ci sono diverse possibilità per la scelta della misura di prossimità, come suggerito in\cite{tan}. 

\subsubsection{K-means\texorpdfstring{++}{}}
La differenza principale tra K-means++ e il K-means classico consiste nell'inizializzazione dell'algoritmo, ovvero nella scelta dei $K$ centroidi iniziali. La successione dei passi, infatti, è la seguente: 
\begin{enumerate}
\item Si sceglie il primo centroide $c_{1}$ in modo casuale tra tutti i punti (nel nostro caso le parole).
\item Per ogni altro elemento del dataset, $x \in \chi$, si calcola la distanza $dist(x,c_{i})$ tra $x$ e il centroide più vicino tra quelli già selezionati.
\item Un elemento generico $x$ viene quindi scelto come centroide con probabilità pari a $\frac{dist(x,c_{i})^2}{\sum\nolimits_{x \in \chi} dist(x,c_{i})^2} $.
\item Si ripetono gli step 2 e 3 finchè non vengono scelti $k$ centroidi.
\item Avendo raggiunto un insieme di $K$ centroidi, si può procedere con il K-means classico.
\end{enumerate}
Con l'inizializzazione suggerita da K-means++, l'algoritmo trova una soluzione in $O(log\;k)$ 

Nel nostro caso, il dataset è costituito dall'insieme delle $n$ parole $W=\{w_{1},...,w_{n}\}$. Come misura di prossimità è stata utilizzata la distanza tra le parole ottenuta dalla matrice di similarità. Il clustering quindi non viene eseguito sul disegno finale, cioè in base al posizionamento sul piano delle parole, ma a partire dalla loro similarità. Ne segue che il raggruppamento delle parole è logico, non geometrico. La funzione obiettivo da minimizzare diventa dunque $\phi = \sum\limits_{i=1}^K \sum\nolimits_{w \in C_{i}} (1 - sim_{w,c_{i}})$. 
...
\item $w_{i} \in P$: è il caso più complesso dei tre. Il termine $w_{i}$ compare in entrambi i layout. Indicati con $\rho_{i}^{k-1}$ il punteggio di $w_{i}$ in $\Gamma^{k-1}$, con $\rho_{i}^{k}$ il punteggio di $w_{i}$ in $\Gamma^{k}$ e con $\Delta_{\rho_{i}}^{k,k-1} = \vert\rho_{i}^{k}-\rho_{i}^{k-1}\vert$, se:
\begin{itemize}
...
\end{itemize}
In base al punteggio corrente, ad ogni frame la dimensione del bounding box di $w_{i}$ viene opportunamente aggiornata. \\ Inoltre, in questo caso, le parole vengono anche traslate, quindi le posizioni dei rispettivi rettangoli variano. Siano dunque $\tau_{i}^{k}=(x_{i}^{k},y_{i}^{k})$ il centro di $w_{i} $ in $\Gamma^{k}$, $\tau_{i}^{k-1}=(x_{i}^{k-1},y_{i}^{k-1})$ il centro di $w_{i} $ in $\Gamma^{k-1}$ e $\,\delta_{\tau_{i}}^{k,k-1} = \vert\vert \tau_{i}^{k} - \tau_{i}^{k-1} \vert\vert$, ovvero la distanza tra $\tau_{i}^{k}$ e $\tau_{i}^{k-1}$. Ad ogni frame, il rettangolo viene traslato, in valore assoluto, di una quantità pari a $\delta_{\tau_{i}}^{k,k-1}/(N+1)$. La direzione dello spostamento dipende dal relativo posizionamento di $\tau_{i}^{k-1}$ in $\Gamma^{k-1}$ e di $\tau_{i}^{k}$ in $\Gamma^{k}$. Abbiamo in generale quattro casi diversi, come indicato in figura \ref{fig:morphcommon}.
\end{enumerate}
\begin{figure}
\centering
\subfigure[]
