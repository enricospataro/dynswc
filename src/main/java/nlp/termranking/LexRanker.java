 ...
 * Erkan & Radev '04, made by Koburov et al.
 */
public class LexRanker {
     ...
     */
    public static <T extends Similar<T>> LexRankResults<T> rank(List<T> data, double similarityThreshold, boolean continuous) {
        LexRankResults<T> results = new LexRankResults<T>();
        if (data.size()==0) return results;
    
        double[][] similarities = similarityMatrix(data);
        double[][] transitionProbabilities = transitionProbabilities(similarities,similarityThreshold,continuous);
        ...
        for (int i=0; i<data.size(); ++i) {
            for (int j=0; j<data.size(); ++j) {
                if (transitionProbabilities[i][j] > 0) {
                    ...
                    if (neighborList == null) neighborList = new ArrayList<T>();
                    neighborList.add(data.get(j));
                    results.neighbors.put(data.get(i), neighborList);
                }
            }
        }
        double[] rankings = powerIteration(transitionProbabilities,data.size(),0.001,1000);

        // Now that we have the LexRank scores, arrange them for the results.
        List<RankPair<T>> tempList = new ArrayList<>();
        for (int i=0; i<data.size(); ++i) {
     ...
     */
    private static double[][] transitionProbabilities(double[][] similarities,double similarityThreshold,boolean continuous) {
    ...
    private static double[][] multMatrix(double[][] first, double[][] second) {
        if (first.length == 0 || second.length == 0) return null;    
        if (first[0].length != second.length) return null;
     ...
     */
    private static double[] powerIteration(double[][] stochasticMatrix, int size, double epsilon, int maxIterations) {
        ...
        for (int i=0; i<maxIterations; ++i) {
            ...
            for(int a=0; a<currentVector.length; a++) currentVector[a][0] *= dampingFactor;
            currentVector = multMatrix(currentMatrix, currentVector);
            for(int a=0; a<currentVector.length; a++) currentVector[a][0] += (1-dampingFactor)/size;
            double error = 0;
            for (int j=0; j<size; ++j) error += Math.pow(currentVector[j][0]-previousVector[j][0], 2);
            if (error < Math.pow(epsilon, 2)) break;
        }
