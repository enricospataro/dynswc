...
public class KMeansStrategy implements ClusterStrategy {
    ...
    public KMeansStrategy(int K)  {this.K = K;}
    ...
    private List<Word> chooseCenters(int K)  {
        List<Word> centers = new ArrayList<>();
    ...
    private Word getNextMean(List<Word> means) {
        ...
        if(minDist.get(p) < 1e-6)  return null;
    ...
    private ClusterResult groupPoints(List<Word> means) {
        ...
        List<Word> median = new ArrayList<>();
        ...
        for(int it=0;it<10;it++)  {
            ...
            for(int i=0;i<median.size();i++)  {
                ...
                if(!median.get(i).equals(newMedian))  progress = true;
    ...
    private void updateClusters(ClusterResult ci)  {
        ...
        for(int t=0;t < 30 && progress;t++) {
            ...
            for(int i=0;i<words.size();i++) {
                ...
                if(bestCluster != ci.getCluster(v)) ci.moveVertex(v, bestCluster);
             
                if(bestModularity > oldModularity + 1e-7)  progress = true;
            }
        }
    }
}
