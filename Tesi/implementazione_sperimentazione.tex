...
Il mantenimento della mappa mentale tra due disegni consecutivi (ovvero la stabilità spaziale) è calcolato con questa metrica. Dati due disegni consecutivi $\Gamma^{k-1}$ e $\Gamma^{k}$, con $k \in \{2,...,K\}$, indichiamo con
$P = \{w_{1},...,w_{p}\}$ l'insieme delle $p$ parole comuni a $\Gamma^{k-1}$ e $\Gamma^{k}$. Denotiamo con $\sigma(w_{i}^{k},w_{i}^{k-1})$, $w_{i} \in P$, la variazione di posizione della parola $w_{i}$ in $\Gamma^{k}$ rispetto a $\Gamma^{k-1}$. Sia inoltre $D$ la diagonale del più grande bounding box che contiene la word cloud. La metrica è definita come:
\begin{equation}
...
\end{itemize}
Eccetto per gli ultimi due valori, tutti i tempi sono stati mediati sul numero di campionamenti del testo.
...
\midrule
$20$ & $0.55 \mu s$ & $0.34 \mu s$& $0.36 \mu s$\\
$40$ & $1.65 \mu s$ & $0.98 \mu s$ & $1.08 \mu s$ \\
$60$ & $3 \mu s$ & $1.88 \mu s$ & $1.87 \mu s$ \\
\bottomrule
\end{tabular}
...
\midrule
$20$ & $3.93 \cdot 10^{-3} s$ & $0.095s$& $6.43 \cdot 10^{-3} s$\\
